...
- **Proxy server** (Apache, port `8088`): maps the URL prefix `/analisis_ejecutivos/` to the app machine.
...
DJANGO_FORCE_SCRIPT_NAME=/analisis_ejecutivos
...
```
ProxyPass /analisis_ejecutivos/ http://<app machine>:8040/
ProxyPassReverse /analisis_ejecutivos/ http://<app machine>:8040/
```

Also add the redirect for the no-trailing-slash form (`/analisis_ejecutivos` -> `/analisis_ejecutivos/`) exactly as done for `/presupuestos_ap`. Restart Apache. `DJANGO_FORCE_SCRIPT_NAME` is required: Apache strips the prefix before forwarding, so without it every link Django generates would come out unprefixed and 404 through the proxy.
