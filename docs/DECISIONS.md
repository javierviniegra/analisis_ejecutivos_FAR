...
- **Rule: several branches per branch -> always ask how to deliver** (owner, 2026-09-24): one PDF with a page per branch, or one PDF per branch. There is no default: without an answer the form comes back asking. Separate PDFs are downloaded together in a .zip (a browser cannot receive several files from one request). Consolidated or a single branch: nothing to ask.
