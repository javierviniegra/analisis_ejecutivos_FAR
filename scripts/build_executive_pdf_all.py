...
# Cross-repo dependency: this project (Analisis Ejecutivos) reuses the
# Wansoft repo's DB connection helpers (core.database.*, core.config.*)
# instead of duplicating them. Works because both repos live as siblings
# on this machine -- NOT portable to another machine without the same
# folder layout. Revisit in Step 25 (own connection module, or a proper
# installable shared package) instead of assuming this path forever.
WANSOFT_REPO_ROOT = r"C:\Users\JavierViniegra\OneDrive - GRUPO FONDA ARGENTINA\Escritorio\AnalisisRestaurantesBI\Wansoft\Jupyter Notebooks\Python Files"
sys.path.insert(0, WANSOFT_REPO_ROOT)
os.environ["ENV"] = "dev"
...
ANALISIS_EJECUTIVOS_DIR = Path(
    r"C:\Users\JavierViniegra\OneDrive - GRUPO FONDA ARGENTINA\Escritorio\AnalisisRestaurantesBI\Reportes\Analisis Ejecutivos"
)
...
COMPANIES = [
    ...
    },
    {
        ...
        "display": "Aeropuerto",
        "gcc_term": "Aeropuerto",
        "ticket_term": "Aeropuerto",
    },
    {
        "key": "IsabelLaCatolica",
        "display": "Isabel La Católica",
        "gcc_term": "Isabel",
        "ticket_term": "Isabel",
    },
    {
    ...
    {
        "key": "TaqueriaParroquia",
        "display": "Taquería Parroquia",
        "gcc_term": "Parroqu",
        "ticket_term": "Parroqu",
    },
    {
    ...
    },
    {
        "key": "Viaducto",
        "display": "Viaducto",
        "gcc_exact": "Fonda Argentina - Viaducto",
        "ticket_exact": "Viaducto",
    },
    {
        "key": "TaqueriaViaducto",
        "display": "Taquería Viaducto",
        "gcc_exact": "Fonda Argentina - Taqueria Viaducto",
        "ticket_term": "Taquer\u00eda Viaducto",
    },
    {
        "key": "SanJeronimo",
        "display": "San Jerónimo",
        "gcc_term": "Jer",
        "ticket_term": "Jeronimo",
    },
    {
        "key": "Tepeyac",
        "display": "Tepeyac",
    ...
    },
    {
        ...
        "display": "Metepec",
        "gcc_term": "Tollocan",
        "ticket_term": "Tollocan",
    },
    {
        "key": "Versalles",
        "display": "Versalles",
        "gcc_term": "Exhibimex",
        "ticket_term": "Exhibimex",
    },
    {
        "key": "Coyoacan",
        "display": "La Esquina Coyoacán",
        "gcc_term": "Coyoacan",
        "ticket_term": "Coyoac",
    ...
    {
        "key": "CentroMyJ",
        "display": "CentroMyJ (Mario y July)",
        "gcc_term": "Mario",
        "ticket_term": "CentroMyJ",
...
CS_KEY_BY_COMPANY = {
    ...
    "IsabelLaCatolica": "Isabel La Católica",
    "Antenas": "Antenas",
    "TaqueriaParroquia": "Taquería parroquia",
    "ViaVallejo": "Vía Vallejo",
    "Viaducto": "Viaducto",
    "TaqueriaViaducto": "Taquería Viaducto",
...
def resolve_name(cur, table, name_col, sucursal_col_filter, cfg, kind):
    ...
    cur.execute(
        f"SELECT DISTINCT {name_col} FROM {table} WHERE {name_col} LIKE %s",
        (f"%{term}%",),
    )
...
def fetch_ticket_data(cur, ticket_name):
    cur.execute(
        "SELECT MIN(Fecha), MAX(Fecha), COUNT(DISTINCT Mesero) FROM getallordenesbyday_new_venta WHERE Sucursal=%s AND Fecha LIKE '2026-08%%'",
        (ticket_name,),
    )
...
def fetch_wansoft_cpv_merma(cur, gcc_name):
    cur.execute(
        "SELECT * FROM costeomensual WHERE subsidiary_name=%s AND created_date='2026-08-31'",
        (gcc_name,),
    )
...
def fetch_odoo_cost(odoo_company_name):
    ...
    if df.empty:
        return {
            "breakdown": pd.DataFrame(columns=["code", "name", "balance"]),
            "cogs": 0.0,
            "var_inv": 0.0,
        }
...
def _bar_chart(labels, values, title, out_path):
    ...
    plt.title(
        title, fontsize=10.5, color="#10564E", fontweight="bold", loc="left", pad=10
    )
    plt.tight_layout()
...
def build_chart_odoo(g, out_path):
    ...
    _bar_chart(
        prod["name"].tolist(),
        prod["balance"].tolist(),
        f"Costo por categoría — Cuenta 501 (Odoo) — {MES_LABEL}",
        out_path,
    )
...
def build_chart_wansoft(venta_neta, costo_total, utilidad, out_path):
    _bar_chart(
        ["Utilidad Marginal", "Costo Total", "Venta Neta"],
        [utilidad, costo_total, venta_neta],
        f"Venta Neta vs. Costo vs. Utilidad Marginal — {MES_LABEL}",
        out_path,
    )
...
def build_company_report(cfg, cur):
    ...
    gcc_name = resolve_name(
        cur, "getglobalcashclosing", "subsidiary_name", None, cfg, "gcc"
    )
    ticket_name = resolve_name(
        cur, "getallordenesbyday_new_venta", "Sucursal", None, cfg, "ticket"
    )
    ...
    if ticket_name is None:
        ticket = {
            ...
            "min_fecha": None,
            "max_fecha": None,
            "mix_alimentos_pct": None,
            "mix_bebidas_pct": None,
        }
    else:
    ...
    if cfg["is_odoo"]:
        ...
        chart_path = str(HERE / f"chart_{key}.png")
        build_chart_wansoft(
            ventas["venta_neta"], costo_total, wcost["utilidad_marginal"], chart_path
        )
    ...
    cheque_prom = (
        ventas["venta_bruta"] / ventas["clientes"] if ventas["clientes"] else 0
    )
    ticket_prom = ventas["venta_bruta"] / ventas["tickets"] if ventas["tickets"] else 0
    venta_por_mesero = (
        ventas["venta_bruta"] / ticket["meseros"] if ticket["meseros"] else None
    )
    clientes_por_mesero = (
        ventas["clientes"] / ticket["meseros"] if ticket["meseros"] else None
    )
    ...
    c.drawString(
        MARGIN_L,
        y,
        f"Fuente: Wansoft (Ventas, Merma, Cortesías, Cancelaciones) y {cost_source_label} (Costo de Productos Vendidos)",
    )
    y -= 26
    ...
    bullets = [
        ...
        (
            ...
            f"+ Merma ${merma:,.2f}"
            + (f" + Variaciones de Inventario ${var_inv:,.2f}" if var_inv else "")
            + ").",
        ),
    ]
    if ticket["mix_alimentos_pct"] is not None:
        bullets.append(
            (
                ...
                + (
                    " (muestra parcial -- ver nota)"
                    if not ticket["full_coverage"]
                    else ""
                ),
            )
        )
    ...
    rows_ventas = [
        ...
        (
            f"# Meseros distintos{cov_note}",
            f"{ticket['meseros']}" if ticket["meseros"] else "N/D",
        ),
        (
            f"Venta por Mesero{cov_note}",
            f"${venta_por_mesero:,.2f}" if venta_por_mesero else "N/D",
        ),
        (
            f"Clientes por Mesero{cov_note}",
            f"{clientes_por_mesero:.1f}" if clientes_por_mesero else "N/D",
        ),
        ("Cheque Promedio (venta/cliente)", f"${cheque_prom:,.2f}"),
        ("Ticket Promedio (venta/ticket)", f"${ticket_prom:,.2f}"),
        (
            f"Mix de ventas -- Alimentos{cov_note}",
            (
                f"{ticket['mix_alimentos_pct']*100:.2f}%"
                if ticket["mix_alimentos_pct"] is not None
                else "N/D"
            ),
        ),
        (
            f"Mix de ventas -- Bebidas{cov_note}",
            (
                f"{ticket['mix_bebidas_pct']*100:.2f}%"
                if ticket["mix_bebidas_pct"] is not None
                else "N/D"
            ),
        ),
    ]
    ...
    rows_costos = [
        (
            ...
            f"${cpv:,.2f}",
            f"{cpv/ventas['venta_neta']*100:.2f}%",
        ),
    ]
    if var_inv is not None:
        rows_costos.append(
            (
                ...
                f"${var_inv:,.2f}",
                f"{var_inv/ventas['venta_neta']*100:.2f}%",
            )
        )
    rows_costos.append(
        ("Costo por Merma", f"${merma:,.2f}", f"{merma/ventas['venta_neta']*100:.2f}%")
    )
    rows_costos.append(
        (
            "COSTO TOTAL OPERATIVO TEÓRICO",
            f"${costo_total:,.2f}",
            f"{cost_pct*100:.2f}%",
        )
    )
    ...
    y = draw_kpi_table(
        ...
        MARGIN_L,
        y,
        f"Costos -- {MES_LABEL} (% s/venta neta)",
        header=("Concepto", "Monto", "% s/venta neta"),
        rows=rows_costos,
        col_widths=[230, 90, 70],
    )
    y -= 8
    ...
    c.drawString(
        MARGIN_L,
        y,
        "Ajustes de Venta -- informativo, a precio de venta (Wansoft, cierre diario)",
    )
    y -= 14
    ajustes_line = (
    ...
    notes = [
        ...
        (
            ...
            + (
                " -- mes completo."
                if ticket["full_coverage"]
                else " -- MUESTRA PARCIAL, no el mes completo. Los indicadores marcados con * están basados en esta ventana, no en los 31 días de agosto."
            )
        ),
    ...
    notes.append(
        "Los % sobre venta se calculan contra Venta Neta (sin IVA), no venta bruta."
    )
    notes.append(
