...
- **On-demand generation (web):** `/reportes/<clave>/generar/` — pick branches (only the user's: `cuentas.models.sucursales_de`), period kind and date (or a free range up to 366 days), per branch or consolidated as the report's `alcance` allows; downloads the PDF. Several branches per branch always ask (no default) for one PDF or one PDF per branch (delivered together in a .zip). Needs the `cuentas.generar_reportes` permission (or superuser) and a generator registered in `central/generadores.py` (today only `comercial-semanal-gerentes`).
- `central/salidas/` — outputs: `pdf_comercial.py` (one letter page per report on the Fonda template: brand background in `recursos/fondo_fonda.jpeg`, DejaVu Sans font bundled with matplotlib).
- `central/motor/fuentes/` — read-only data access: `wansoft.py` (deduplicated daily cash closings by operating day, channel, mix; one batched query per period for all branches), `presupuestos.py` (Costo de Ventas budget vs real from ControlPresupuestos_AP), `conexiones.py` (read-only sessions, 120 s server-side cap per statement). **`FUENTES_ENV`** in `config/.env` chooses where report data is read from (`prod` or `dev`, defaults to `ENV`), independently of the app's own database. Verify with `python manage.py probar_reporte <branches|todas> --tipo <kind> [--fecha | --desde --hasta] [--consolidado]` (prints the Lectura, the table and the footnotes; `--graficas <folder>` also saves the charts as PNG, `--pdf <folder>` the PDF). Branch mapping across systems: `python manage.py cargar_sucursales`.
