...
- **On-demand generation (web):** `/reportes/<clave>/generar/` — pick branches (only the user's: `cuentas.models.sucursales_de`), period kind and date (or a free range up to 366 days), per branch or consolidated as the report's `alcance` allows; downloads the PDF. Needs the `cuentas.generar_reportes` permission (or superuser) and a generator registered in `central/generadores.py` (today only `comercial-semanal-gerentes`).
